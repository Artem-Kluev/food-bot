...
const id = tgWebApp?.initDataUnsafe?.user?.id ?? null
...
export async function userData(requestData: any = null) {
  const dataValue = { initData, id, requsetData: null }

  if (requestData) dataValue.requsetData = { id, ...requestData }
